...
import { Box, Divider, FormControlLabel, Grid, LinearProgress, Paper, Radio, RadioGroup, TextField } from "@mui/material";
...
const Renewal = () => {
        ...
        onSubmit: async values => {
            ...
            mpesa.onSuccess = async () => {
                await updateDoc(doc(db, `licenses/${user.uid}/classes`, values.id), {
                    ...
                    validUntil: values.type === 'pdl' ? moment().add(3, 'months').format("MMMM Do YYYY") : moment().add(3, 'years').format("MMMM Do YYYY")
                });
            };
    ...
    useEffect(() => {
        const getData = async () => {
            await getDocs(collection(db, `licenses/${user.uid}/classes`)).then(res => {
                setExpiredDLs(res.docs.filter(doc => moment(doc.data().validUntil, 'MMMM Do YYYY').isAfter(moment())).map(doc => ({
                    id: doc.id,
                    data: doc.data()
                })));
